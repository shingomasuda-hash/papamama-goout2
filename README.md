...
`src/app/components/StaffSlider.tsx`（embla-carousel）。ループ・自動再生（4.5秒、操作で停止／視差効果を減らす設定では無効）・スワイプ／ドラッグ・ドットクリックに対応。

- スタッフカードは前回LPと同じ素材（`public/images/staff/staff-01〜06.png`）
- 並び替え・追加は `src/app/staff.ts` の `STAFF` 配列を編集（初期表示は3番目＝森行さん、見本と同じ）
- 枝・葉は固定、カードだけがスライドします
